...
class RandomLayer(nn.Module):   #([args.feature_dim, args.class_num], 1024)
    def __init__(self, input_dim_list=[], output_dim=1024):
        super(RandomLayer, self).__init__()
        self.input_num = len(input_dim_list)  #input_num=2
        self.output_dim = output_dim
        self.random_matrix = [torch.randn(input_dim_list[i], output_dim) for i in range(self.input_num)]
    #     random_matrix:list ([160,1024],[9,1024])
    # input_list:features[308,160],softmax_output[308,9]->return_list list2:[308,1024],[308,1024]
    def forward(self, input_list):
        return_list = [torch.mm(input_list[i], self.random_matrix[i]) for i in range(self.input_num)]
        # 将 return_list 的第一个元素除以 self.output_dim 的算术平方根
