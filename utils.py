...
def flip(data):
    y_4 = np.zeros_like(data)  # [610,340,103]
    y_1 = y_4
    y_2 = y_4
    first = np.concatenate((y_1, y_2, y_1), axis=1)
    second = np.concatenate((y_4, data, y_4), axis=1)  # [610,1020,103]
    third = first  # [610,1020,103]
    Data = np.concatenate((first, second, third), axis=0)  # [1830,1020,103]
    return Data
...
def load_data(image_file, label_file):
    ...
    data_all = image_data[data_key]  # dic-> narray , Bo:ndarray(512,217,204)
    mean_per_band = np.mean(data_all, axis=(0, 1))  # 沿着像素点的方向求均值
    std_per_band = np.std(data_all, axis=(0, 1))  # 沿着像素点的方向求标准差

    # 标准化处理
    normalized_data = (data_all - mean_per_band) / std_per_band
    GroundTruth = label_data[label_key]

    [nRow, nColumn, nBand] = data_all.shape
    print(data_key, nRow, nColumn, nBand)

    data = normalized_data.reshape(np.prod(data_all.shape[:2]), np.prod(data_all.shape[2:]))  # (111104,204)
...
def load_data2(image_file):
    image_data = sio.loadmat(image_file)
    label_data = sio.loadmat(image_file)

    data_all = image_data["DC_Sub"]  # dic-> narray , Bo:ndarray(512,217,204)
    GroundTruth = label_data["grd"]

    [nRow, nColumn, nBand] = data_all.shape
    print("DC_sub", nRow, nColumn, nBand)
...
class Task(object):
    ...
    def __init__(self, data, num_classes, shot_num, query_num):
        ...
        for c in class_list:
            ...
            # print("support_datas:", self.support_datas)
